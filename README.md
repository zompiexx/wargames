...
Update 19 June 2022: I am in the process of re-writing the code in C so that it can be easily compiled on various platforms. I am also making it client/server so that when the dialer connects to a system, e.g. WOPR, Pan AM etc. these will actually be hosted on another machine (or can be on the same machine if preferred). I am currently working on the Imsai 8080 sim in C which I will publish shortly, but I have included an early sneak preview in the repository (no validation checks written into the code yet) if you want to play with it.
